...
- An opt-in MCP server is shipped inside this desktop repo under `packages/mcp-server/`. It runs as a local stdio subprocess, reuses the same AI tool schemas, opens the local Daylens SQLite database read-only, and exposes a Settings config snippet for MCP clients such as Claude Desktop or Cursor (`packages/mcp-server/src/index.ts:1-67`, `src/main/services/mcpServer.ts:19-57`, `src/renderer/views/Settings.tsx:1348-1428`).
...
- MCP client connectivity and answer quality with real Claude Desktop, Cursor, or Claude Code sessions.
...
- `npm run build:mcp` builds the bundled MCP stdio server used by packaged Daylens builds. `npm run build:all` runs this automatically.
