...
test('the chat surfaces stay up while the provider verdict is unresolved', () => {
  // hasApiKey is boolean | null: only a resolved `false` swaps the composer
  // for the connect prompt.
  assert.match(workspace, /const chatVisible = hasApiKey \?\? true/)
  assert.match(workspace, /\{chatVisible && !providerFailed && \(hasMessages \|\| threadLoading\) && \(/)
  assert.match(workspace, /\) : settings && !hasApiKey \? \(/)
  // A send typed into that composer waits for the verdict instead of hitting
  // the hook's access check and vanishing.
