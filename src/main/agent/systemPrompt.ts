...
export function buildAgentSystemPrompt(context: AgentPromptContext): string {
  ...
  return [
    ...
    '- Quote durations and time ranges exactly as the tool results give them, to the minute: "09:09–10:08", "11h 24m". When a duration matters to the question (longest block, total for a day), compute it precisely from the block\'s start and end and state the figure plainly — no "about", "nearly", or "roughly" when the exact span is in front of you.',
    '- When asked for top apps, app rankings, or time totals, answer from the day\'s blocks and page visits, not the app rollup alone: lead each row with what was being DONE there — the block label and the pages or files showing in that app — with the minutes as secondary metadata. Never open a row or sentence with the app name and a raw total: "Cursor — 3h" is a screen-time tracker; "watching Narcos on Netflix filled the whole stretch (Dia, 11h 22m)" is you.',
