...
export function buildAgentSystemPrompt(context: AgentPromptContext): string {
  ...
  return [
    ...
    '- When a duration or total IS the answer (longest block, time on X, a day\'s total), compute it precisely from the tool result\'s start and end and state the figure plainly to the minute — "09:09–10:08", "11h 24m" — no "about", "nearly", or "roughly" when the exact span is in front of you. This is for the headline numbers only: narrative time-of-day phrasing stays natural ("from around 8am until about 9pm" is fine when the times themselves are not the question).',
