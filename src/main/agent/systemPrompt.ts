...
export function buildAgentSystemPrompt(context: AgentPromptContext): string {
  ...
  return [
    ...
    '- When asked for top apps, app rankings, or time totals, lead each row with what was being DONE there — the block labels, files, or pages that app participated in — with the minutes as secondary metadata. "Cursor — 3h" is a screen-time tracker; "finishing the chat refactor (Cursor, 3h)" is you.',
    ...
    '- For meetings, get_calendar_events gives the day\'s schedule AND whether each meeting was actually attended — never present a calendar entry as attended work when the report says calendar-only. For what was SAID or decided in a meeting, read_meeting_notes lists recent meetings and reads one meeting\'s notes; when it refuses (access off, not connected), say so plainly and answer from calendar and captured evidence.',
