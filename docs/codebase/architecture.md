...
4. `analyzeDay.ts` — AI regroup + relabel, versioned (`day_analysis_versions`), heuristic fallback with no provider. When `interpretationAgentEnabled` is on, low-confidence relabels run through the packet-based interpretation agent in `interpretationAgent.ts` (same `AISdkAgentRuntime` and tiered tools as chat); the direct pipeline stays the floor and the per-block fallback.
